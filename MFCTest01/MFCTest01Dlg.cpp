...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_BUTTON1, btnTest);
...
int flag = 0;
// GetDlgItem() = 대화상자에 생성된 컨트롤의 주소를 얻는다
// GetWindowTextW() = 윈도우의 텍스트를 가져오는 데 사용
...
{	
	if (flag == 0)	// 첫번째 클릭 
	{
		...
		((CEdit*)(GetDlgItem(IDC_EDIT1)))->GetWindowTextW(str); // 아랫줄과 같은 의미와 기능
	...
	{
		//((CStatic*)GetDlgItem(IDC_STATIC))->SetWindowTextW(L"안녕히가세요");
